        ...
        jwt.verify(token, process.env.JWT_SECRET, async (error, payload) => {
            if (error) {
                if (error.message === "jwt expired") {
                    ...
                    } else if (tok_data.type === "Teacher") {
                        user = await teacher_model.findOne({ token: tok_data._id })
                    } else {
                    ...
                    let newToken;
                    if (tok_data.type === "Student") {
                        newToken = generate_token({ _id: user._id, class: user.class, regNo: user.regNo, type: "Student" })
                    }
                    else if (tok_data.type === "Teacher") {
                        newToken = generate_token({ _id: user._id, type: "Teacher" })
                    }
                    else {
                        newToken = generate_token({ type: "Admin", _id: user._id })
                    }
                ...
                if (data.type === "Student") {
                    ...
                    user = await teacher_model.findById({ _id: data._id })
                }
                else {
